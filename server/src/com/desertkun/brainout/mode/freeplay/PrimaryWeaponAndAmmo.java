...
public class PrimaryWeaponAndAmmo
{
    ...
    {
        // a primary gun and some ammo into it
        InstrumentSlotItem slot = free.getRandomPrimaryWeapon();
            ...
            if (sw != null)
            {
                ...
                if (bullet != null)
                {
                    switch (MathUtils.random(6))
                    {
                        ...
                        {
                            // two mags
                            cnt.putConsumable(getClipSize(weapon.getPrimaryProperties().getClipSize() * 2),
                        ...
                        {
                            // full mag
                            cnt.putConsumable(getClipSize(weapon.getPrimaryProperties().getClipSize()),
                                    new DefaultConsumableItem(bullet));
                            break;
                        }
                    }
    ...
    private static int getClipSize(int clipSize)
    {
        if (clipSize < 8)
            return 8;
